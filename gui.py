...
class Ui_MainWindow(object):
    ...
    # Function that runs when program starts and whenever 'new suggestion' button is pressed. It updates the movie
    # information.
    def newSuggestion(self):
            global movie
            movie = self.controller.getRandomMovie(apiName)
            try:
                ...
                link = self.controller.getTrailerLink(movie['id'])
                self.trailerLink.clicked.connect(lambda: self.open_webbrowser(link))

            except TypeError:
                print("TypeError")
                self.newSuggestion()

    def open_webbrowser(self, link):
        webbrowser.open(link)

    # Function that runs when 'already seen' button is pressed. Calls on controller to add movie to 'seen'
    def addToSeen(self):
            self.controller.addToSeen(movie)
            self.newSuggestion()

    # Function that runs when 'add to watchlist' button is pressed. Calls on controller to add movie to 'watchlist'
    def addToWatchlist(self):
            self.controller.addToWatchlist(movie)
            self.newSuggestion()

    # Function that runs when 'Top 250' button or 'Most popular' is pressed. Changes page to movie screen.
    ...
    # Function that runs when 'MoviePicker' logo is pressed. Changes page to start screen.
    def goToStart(self):
            self.stackedWidget.setCurrentWidget(self.start)

    # Function that populates lists on 'Seen' and 'Watchlist' page.
    def populateList(self, listName):
        ...
        global movies
        if listName == 'seen':
            movies = self.controller.getAlreadySeen()

        elif listName == 'watchlist':
            movies = self.controller.getWatchlist()
        ...
        for movie in movies:
            ...
            self.listItem.setStyleSheet("background-color: #023d5e;")
    ...
    # Function that runs when 'Seen' or 'Watchlist' button is pressed. Changes page to Lists screen.
    def goToList(self, list):
        self.populateList(list)
        self.stackedWidget.setCurrentWidget(self.lists)

    # Function that runs when 'x' button is pressed to delete an item from the list.
    def deleteItem(self, listName, id):
        if listName == 'watchlist':
            self.controller.removeFromWatchList(id)
        elif listName == 'seen':
            self.controller.removeFromSeen(id)
